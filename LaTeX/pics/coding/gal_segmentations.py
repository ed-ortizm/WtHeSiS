...
def plot_seg_img(img,segmented_img, img_name, output_path='.',
    ...
    plt.tight_layout()
    plt.savefig(f'{output_path}/segmented_{img_name}.png')
    plt.savefig(f'{output_path}/segmented_{img_name}.pdf')
...
for gal_path in gal_paths:
    ...
    img = plt.imread(f'{gal_path}')

    if img.shape[-1] == 4:

        img = img[:, :, :3]
    print(f'The shape of {gal_name} is {img.shape}')

    print(f'SLIC - K-Means based image segmentation')
    segments_slic = slic(img, n_segments=100, compactness=10, sigma=1)
    print(f"Sahape of slic segmentation: {segments_slic.shape}")
